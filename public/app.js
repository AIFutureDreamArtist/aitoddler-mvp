...
function setupVoiceInput() {
  ...
  recognition.onresult = (event) => {
    ...
    const goalInput = document.querySelector("#pilot-goal");
    if (goalInput) {
    ...
    <div>
      <div style="display:flex; align-items:center; gap:1rem; margin-bottom:1.5rem">
        <div>
          ...
          <p style="margin:0.3rem 0 0; color:var(--muted); font-size:1.1rem">Veilige lokale AI agent • Slimme multi-file scaffolding + Voice</p>
        </div>
      ...
      <!-- Goal Launcher with Voice -->
      <div class="control-panel" style="margin-bottom:1.5rem; border:2px solid #0b7a53">
        <h2 style="margin-top:0; color:#0b7a53">🚀 Slimme Project Scaffolding</h2>
        <div style="position:relative">
          <textarea id="pilot-goal" placeholder="Bouw een volledige moderne SaaS app met landing, dashboard, auth en settings (meerdere bestanden)" style="min-height:100px; padding-right:50px"></textarea>
        ...
        <div style="margin:0.8rem 0">
          <div class="segmented" id="pilot-focus">
            <button class="selected" data-focus="ui">UI + Multi-File Scaffolding</button>
            <button data-focus="code">Full Code Project</button>
            <button data-focus="research">Research + Report</button>
          </div>
        </div>

        <button id="pilot-launch" class="primary" style="width:100%; padding:1rem; font-size:1.1rem; background:#0b7a53">
          Launch Slimme Multi-File Scaffolding
        </button>
        <div style="margin-top:0.5rem; font-size:0.8rem; color:var(--muted); text-align:center">
          Genereert automatisch meerdere bestanden + proposals
        </div>
      </div>
        ...
        <div class="wide-panel">
          ...
          ${activeTasks.length ? activeTasks.map(t => `
            <div class="row" style="padding:0.9rem; border-left:5px solid #0b7a53">
              ...
              <div style="font-size:0.8rem; color:var(--muted); margin-top:0.3rem">${t.focus} • ${t.status}</div>
            </div>
          `).join('') : '<div class="empty">Geen actieve agents</div>'}
        </div>

        <div class="wide-panel">
          <div class="section-title"><h2>Approval Queue (Diff)</h2><span>${pending.length}</span></div>
          ${pending.length ? pending.map(p => `
            <div style="padding:1rem; background:white; border:1px solid #d9e2d6; border-radius:8px; margin-bottom:0.8rem">
              ...
              <pre style="background:#f4f7f1; padding:0.7rem; border-radius:6px; font-size:0.75rem; max-height:110px; overflow:auto">${escapeHtml(p.diff || 'Diff preview')}</pre>
              <button data-approve="${p.id}" style="margin-top:0.5rem; background:#0b7a53; color:white; border:0; padding:0.5rem 1.2rem; border-radius:6px">Approve & Write</button>
            </div>
...
function bindViewEvents() {
  ...
  if (voiceBtn) {
    ...
    voiceBtn.addEventListener("click", () => {
      if (!recognition) {
        alert("Voice input wordt niet ondersteund in deze browser (probeer Chrome/Edge)");
        return;
      }
  ...
  if (pilotBtn) {
    pilotBtn.addEventListener("click", async () => {
      ...
      pilotBtn.textContent = "Scaffolding meerdere bestanden...";
      ...
      await fetch("/api/chat", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          message: `PilotDirector, voer slimme multi-file scaffolding uit voor: ${goal}. Genereer automatisch meerdere relevante bestanden als proposals met diff. Vraag approval voor elke write. Maak het project compleet.` 
        })
      });
      ...
      await fetch("/api/experiments/run", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ hypothesis: `Smart multi-file project scaffolding: ${goal}`, focus: "stewardship" })
      });

      await refresh();
      pilotBtn.textContent = "Launch Slimme Multi-File Scaffolding";
